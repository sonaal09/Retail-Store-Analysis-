# Walmart Sales Dashboard (Improved with Home Icon for Overview)
import pandas as pd
import numpy as np
...
# Keep raw copy for filtering reference
df_raw = df.copy()
...
with col2: 
    ...
    st.markdown(f"""
        <div style="
            ...
            text-align: center;
        ">
            <p style="margin:0; font-size: 18px;">💰 <strong>Total Revenue</strong></p>
...
with col3:
    ...
    st.markdown(f"""
        <div style="
            ...
            text-align: center;
        ">
            <p style="margin:0; font-size: 18px;">⭐ <strong>Average Rating</strong></p>
...
with col4:
    ...
    st.markdown(f"""
        <div style="
            ...
            text-align: center;
        ">
            <p style="margin:0; font-size: 18px;">📦 <strong>Total Transaction</strong></p>
...
with st.container():
    st.markdown("### 🧩 Understanding Sales Metric Relationships (Advanced Insight)")

    numeric_df = filtered_df[['unit_price', 'quantity', 'rating', 'total_price', 'profit_margin']]
    ...
    fig_corr = px.imshow(
        ...
        title='Correlation Heatmap of Sales Features'
    )
    ...
    st.expander("🔍 **View Correlation Matrix Data**").write(corr_matrix)
    st.download_button("📥 Download Correlation Data",
...
with st.expander('🔍 **View Raw Data (Interactive)**'):
